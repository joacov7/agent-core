...
// Corre el MISMO catálogo contra el adapter del marketplace (tenant = un vendedor).
// `npm run build && node apps/marketplace/dist/demo.js`
async function main(): Promise<void> {
  const app = crearApp();
  const ctx: TenantCtx = { tenantId: "vendedor1", requestId: "demo-mkt", now: () => new Date("2026-09-14T12:00:00.000Z") };
  ...
  console.log(`Capacidades del vendedor: ${capacidades.join(", ")}`);
